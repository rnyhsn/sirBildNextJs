...
const EditorialTeamPage = async () => {
  const resp = await getMembers('editor');
  return (
    <div className="">
            ...
            resp.success && resp.statusCode === 200 && resp.payload.map((member: any, i: number) => {
              ...
              return <div key={i} className="flex gap-4 items-center">
                ...
                <div>
                    ...
                    <h2 className="text-sm dark:text-gray-300 text-gray-700"> {member.academy} </h2>
                    <h2 className="text-sm text-gray-400"> <code>{member?.location}</code> </h2>
                    <Link href={`mailto:${member.email}`} className="text-blue-500 text-sm"> email: {member.email} </Link>
                  </div>
              </div>
            })
          }
